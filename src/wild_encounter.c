...
void CreateWildMon(enum Species species, u8 level)
{
    ...
    if (IsNuzlockeActive() && FlagGet(FLAG_SYS_POKEDEX_GET) && !FlagGet(FLAG_NUZLOCKE_SKIP_ENCOUNTER))
    {
        ...
        {
            u32 nuzPersonality = GetMonData(&gParties[B_TRAINER_OPPONENT_A][0], MON_DATA_PERSONALITY);
            u32 nuzOtId = GetMonData(&gParties[B_TRAINER_OPPONENT_A][0], MON_DATA_OT_ID);
